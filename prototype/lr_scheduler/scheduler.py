...
class _WarmUpLRScheduler(_LRScheduler):
    ...
    def __init__(self, optimizer, base_lr, warmup_lr, warmup_steps, last_iter=0, reset_steps=0):
    ...
    def _get_warmup_lr(self, outer_lr_scale=None):
        if self.warmup_steps >= 2:
            if self.last_iter < self.warmup_steps:
                step = self.last_iter
                target_lr = (self.warmup_lr - self.base_lr) / \
                    (self.warmup_steps-1) * (step-1) + self.base_lr
                scale = target_lr / self.base_lr
                return [scale * base_lr for base_lr in self.base_lrs]
            elif self.reset_steps >0 and self.last_iter % self.reset_steps < self.warmup_steps:
                step = self.last_iter % self.reset_steps
                target_lr = (self.warmup_lr - self.base_lr) / \
                    (self.warmup_steps-1) * (step-1) + self.base_lr
                ratio = target_lr / self.warmup_lr
                return [outer_lr_scale * ratio * base_lr for base_lr in self.base_lrs]
        else:
            return None
...
class StepLRScheduler(_WarmUpLRScheduler):
    ...
    def __init__(self, optimizer, lr_steps, lr_mults, base_lr, warmup_lr, warmup_steps, max_iter, last_iter=0, reset_steps=0):
        super(StepLRScheduler, self).__init__(
            optimizer, base_lr, warmup_lr, warmup_steps, last_iter, reset_steps)
...
class StepDecayLRScheduler(_WarmUpLRScheduler):
    ...
    def __init__(self, optimizer, step_size, decay, base_lr, warmup_lr, warmup_steps, max_iter, last_iter=0, reset_steps=0):
...
class CosineLRScheduler(_WarmUpLRScheduler):
    ...
    def __init__(self, optimizer, max_iter, min_lr, base_lr, warmup_lr, warmup_steps, last_iter=0, reset_steps=0):
    ...
    def _get_new_lr(self):
        ...
        scale = target_lr / self.base_lr
        warmup_lr = self._get_warmup_lr(outer_lr_scale=scale)
        if warmup_lr is not None:
            return warmup_lr
        return [scale*base_lr for base_lr in self.base_lrs]
...
class PolynomialLRScheduler(_WarmUpLRScheduler):
    ...
    def __init__(self, optimizer, power, max_iter, base_lr, warmup_lr, warmup_steps, last_iter=0, reset_steps=0):
