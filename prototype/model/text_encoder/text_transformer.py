...
class TokenizedOutput:
    def __init__(self, output) -> None:
        self.out1, self.out2 = output
    def cuda(self):
        return self.out1.cuda(), self.out2.cuda()
...
class TextTransformer(nn.Module):
    ...
    def wrap_tokenize(self, text):
        output = self.tokenize(text)
        output = TokenizedOutput(output)
        return output
        
    # TODO Bert-Masking Not Implemented for most tokenizers
    def forward(self, text, mask_type=None, return_dense=False, return_raw_feature=False, return_padmask=False, return_att=False, raw_text=True):
        ...
        if self.text_encode_type == 'Transformer':
            ...
            if raw_text:
                ...
                if mask_type is not None: texts, labels, pad_mask = text
                else: texts, pad_mask = text
            #------feed to transformer
