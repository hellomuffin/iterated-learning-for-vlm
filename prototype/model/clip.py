...
class CLIP_sp(CLIP):
    def __init__(self,image_encode, text_encode, use_allgather):
        super().__init__(image_encode, text_encode, use_allgather)
        apply_spectral_norm_to_linear_layers(self)
        print("apply spectrum norm to the model!")
...
def clip_vitb32_sp(**kwargs):
    """'
    Constructs a clip_ViT_B32 model.
    """
    image_encode = visual_transformer_B32(**kwargs['image_encode'])
    text_encode = text_transformers(**kwargs['text_encode'])
    model = CLIP_sp(image_encode,text_encode,**kwargs['clip'])
    return model
